...
const SUBTABS: { id: 'motion' | 'music' | 'canvas' | 'proj'; labelKey: string }[] = [
  { id: 'motion', labelKey: 'fx.subMotion' },
  { id: 'music', labelKey: 'fx.subMusic' },
  { id: 'canvas', labelKey: 'fx.subCanvas' },
  { id: 'proj', labelKey: 'fx.subProj' }
]

/** 动效面板（0.5.0）：背景 / 主图 / 文本入场 / 全局后期 / 片头片尾（全部默认关闭）——
 * 内容较多 → 二级选项卡分栏（画面动效 / 音乐响应 / 后期特效 / 片头片尾与音频） */
export function FxPanel(props: FxPanelProps): React.JSX.Element {
  ...
  const [subTab, setSubTab] = useState<'motion' | 'music' | 'canvas' | 'proj'>('motion')
  return (
      ...
      <div className="fx-subtabs">
        {SUBTABS.map((s) => (
          <button
            key={s.id}
            type="button"
            className={'fx-subtab' + (subTab === s.id ? ' active' : '')}
