...
- [ ] T10: 端到端与回归（**T10a ✓ --smoke-time；T10b 慢盘探针 ✓；60min 1080p 内存验收：首轮主进程 OOM（音频流式解码 Buffer.concat 无背压 → 1.27GB f32 排队至爆）→ e051dea 修复（池化批拼接 + stdout pause/resume 真背压 + 收尾等泵；附带曾致音频截断的竞态修复）→ 重跑中（pwsh-27，~3×60min 变体≈50-60min）；**4K 验收按用户决策移出 1.0.0**（软件本版不主打 4K）；剩余：4GB VM 全流程（独立窗口）**）——typecheck/lint/测试（timeline 引擎单测）/build；smoke 扩展：time-smoke（多片段布局 → 播放 t 与导出同帧像素一致校验 + 关键帧值正确）、流式写盘冒烟（正常导出产物 ffprobe 一致 + 慢盘队列深度探针）、**内存验收：60min 1080p / 10min 4K 导出渲染进程堆峰值 <2GB；4GB 虚拟机全流程通过**、smoke-project（段布局/关键帧 保存→还原）、既有三项 smoke 全绿；WYSIWYG 帧一致性（预览 seek(t) 截图 vs 导出抽帧 t）
