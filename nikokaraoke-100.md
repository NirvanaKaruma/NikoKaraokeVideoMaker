...
- 状态：T1–T6 + T7a 完成；**T7b 流式写盘**（mp4-muxer StreamTarget + MessagePort 零拷贝 + 三级背压 + 错误/取消协议）与 T8–T11 待做。
