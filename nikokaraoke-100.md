...
- 用户验收轮（1.0.1）：① 文案精简（A–J 按用户裁决：删除 B/C 整条、A=「正值视频向前偏移…」、4K FAQ=「不建议使用4K设置」、themeNote 整删）；② 时间轴开关按钮（顶栏）；③ 分割修复（无片段时=整首切两段）；④ 主题系统（深色默认+按钮统一图二风格 / 浅色可切换）；⑤ 关键帧修复（无轨道时添加自动建轨——此前静默失效）+ 附加图层动态关键帧目录（overlayLayers.<i>.rect/opacity/fx，标签=图层{i}·字段；已知限制：层删除/重排后旧路径失配）+ 段块间 2px 视觉缝隙；⑥ commit 待做。
