...
- [x] T22: ffmpeg 来源矩阵测试 + 托管安装实测（真下载或本地镜像）→ 写入 docs/TEST.md
...
- [x] T26: electron-builder（NSIS + portable，不捆 ffmpeg）
- [x] T27: 无 Node 干净目录 portable 实测 + 体积记录 → docs/TEST.md
...
- M4 反馈新增：GPU 加速检测（硬件/软件 30 帧实测基准 + 自动持久化推荐 + 导出按推荐探测）；本机实测软件更快 → 自动选软件（docs/TEST.md §5.5）。
